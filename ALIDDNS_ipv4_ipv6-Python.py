...
from urllib.request import Request, urlopen  # 修改为用 Request 对象
import json
...
print("本程序版权属于zeruns，博客：https://blog.zeruns.tech")  # 仅移动了位置
...
if ipv4_flag == 1:
    ...
    url = "https://api-ipv4.ip.sb/ip"  # 构造请求头（模拟 Chrome 浏览器的 User-Agent，也可换其他浏览器 UA）
    headers = {    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36"}  # 用 Request 对象封装 URL 和请求头
    req = Request(url, headers=headers)  # 发送请求并读取响应
    response = urlopen(req)
    ipv4 = response.read().decode('utf-8')  # 若返回是字符串，需解码（如 utf-8）
...
if ipv6_flag == 1:
    ...
    url = "https://api-ipv6.ip.sb/ip"  # 构造请求头（模拟 Chrome 浏览器的 User-Agent，也可换其他浏览器 UA）
    headers = {    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36"}  # 用 Request 对象封装 URL 和请求头
    req = Request(url, headers=headers)  # 发送请求并读取响应
    response = urlopen(req)
    ipv6 = response.read().decode('utf-8')  # 若返回是字符串，需解码（如 utf-8）
